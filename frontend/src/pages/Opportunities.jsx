  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          <h1 className="text-4xl font-black text-gray-900 dark:text-white tracking-tight">Recycling Initiatives</h1>
      ...
      <div className="flex flex-col lg:flex-row gap-4 items-center bg-white dark:bg-gray-900 p-4 rounded-[2rem] shadow-sm border border-gray-100 dark:border-gray-800 transition-colors">
        ...
        <div className="flex items-center space-x-3 w-full lg:w-auto">
          ...
          <select
            className="bg-gray-50 dark:bg-gray-800 border-none rounded-2xl py-4 px-6 focus:ring-2 focus:ring-green-500 transition-all outline-none font-bold text-gray-700 dark:text-gray-200 appearance-none cursor-pointer pr-12"
            value={status}
      ...
      {loading ? (
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          <AnimatePresence>
            {filtered.length > 0 ? (
              filtered.map((o, idx) => {
                const isApplied = o.applicants?.some(app => app.user === userId || app === userId);
                ...
                return (
                  ...
                  >
                    <div className="relative h-56 overflow-hidden">
                      ...
                      {isApplied && (
                        <div className="absolute top-6 right-6 bg-white/90 dark:bg-gray-800/90 backdrop-blur-md text-green-600 dark:text-green-400 px-4 py-2 rounded-2xl flex items-center space-x-2 shadow-xl border border-white/20 dark:border-gray-700">
                          <FiCheckCircle className="text-lg" />
                          <span className="text-xs font-black uppercase tracking-widest leading-none">Applied</span>
                        </div>
                      )}
                    ...
                    <div className="p-8 flex-1 flex flex-col space-y-4">
                      <div className="flex justify-between items-start">
                         <span className="bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest">{o.wasteType || "Community"}</span>
                         <span className={`text-[10px] font-black uppercase tracking-widest ${o.status === 'Open' ? 'text-green-500' : 'text-red-400'}`}>● {o.status}</span>
                      </div>
                      ...
                      <div className="pt-4 mt-auto border-t border-gray-50 dark:border-gray-800 flex items-center justify-between text-xs font-bold text-gray-400">
